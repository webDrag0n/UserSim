...
import { Card } from '../components/StateBars'
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-semibold text-zinc-200">世界状态知识图谱 · t={curT}</span>
        <div className="flex gap-3 text-[10px] text-zinc-500">
          <span>━━ 位置在（当前）</span>
          <span className="text-pink-400">━━ 经历系列</span>
          <span className="text-zinc-500">┅ 位于/观测</span>
        </div>
      </div>
      <svg viewBox="0 0 620 420" className="w-full">
        ...
        <line x1={assistantPos.x + 30} y1={assistantPos.y} x2={CX - 34} y2={CY}
          stroke="#71717a" strokeWidth={1.2} strokeDasharray="4 4" className="edge" opacity={0.6} />
        <text x={(assistantPos.x + CX) / 2 - 10} y={CY - 8} fill="#71717a" fontSize={9}>观测/辅助</text>
          ...
          return (
            <g key={n}>
              <line x1={CX} y1={CY} x2={p.x} y2={p.y}
                stroke={active ? '#22d3ee' : '#3f3f46'} strokeWidth={active ? 2.5 : 1}
                className={`edge ${active ? 'flow' : ''}`} opacity={active ? 1 : 0.35} />
              {active && (
                <text x={(CX + p.x) / 2} y={(CY + p.y) / 2 - 6} fill="#22d3ee" fontSize={10} textAnchor="middle">位置在</text>
              )}
            </g>
          )
        })}
        ...
        {series.filter((s) => s.start_day * 4 <= curT && curT < s.end_day * 4).map((s) => {
          ...
          return (
            <g key={s.id}>
              <line x1={CX} y1={CY} x2={p.x} y2={p.y} stroke="#f472b6" strokeWidth={2}
                className="edge flow" opacity={0.9} />
              <text x={(CX + p.x) / 2} y={(CY + p.y) / 2 - 6} fill="#f472b6" fontSize={9} textAnchor="middle">经历中</text>
            </g>
          )
        })}
        ...
        {eventPos.map(({ e, x, y }) => {
          ...
          return (
            <g key={e.id}>
              <line x1={CX} y1={CY} x2={x} y2={y} stroke="#a78bfa" strokeWidth={1.5} className="edge" opacity={0.8} />
              {lp && <line x1={x} y1={y} x2={lp.x} y2={lp.y} stroke="#3f3f46" strokeWidth={1} strokeDasharray="3 3" className="edge" opacity={0.5} />}
              <circle cx={x} cy={y} r={7} fill="#a78bfa33" stroke="#a78bfa" className="node" />
              <text x={x} y={y + 18} fill="#c4b5fd" fontSize={8.5} textAnchor="middle">{e.name.slice(0, 6)}</text>
            </g>
          )
        })}
          ...
          return (
            <g key={n} className="node">
              <circle cx={p.x} cy={p.y} r={active ? 16 : 11}
                ...
                stroke={active ? '#22d3ee' : '#52525b'} strokeWidth={active ? 2 : 1} className="node" />
              <text x={p.x} y={p.y + 3.5} fill={active ? '#22d3ee' : '#a1a1aa'} fontSize={active ? 11 : 9.5}
          ...
          return (
            <g key={s.id} className="node">
              <rect x={p.x - 26} y={p.y - 13} width={52} height={26} rx={8}
                fill={activeS ? 'rgba(244,114,182,0.15)' : 'rgba(255,255,255,0.03)'}
                stroke={activeS ? '#f472b6' : '#52525b'} strokeWidth={activeS ? 1.5 : 1} className="node" />
              <text x={p.x} y={p.y + 3.5} fill={activeS ? '#f9a8d4' : '#71717a'} fontSize={9} textAnchor="middle">
                {s.icon}{s.name}
              </text>
            </g>
          )
        ...
        {/* 助手节点 */}
        <g className="node">
          <circle cx={assistantPos.x} cy={assistantPos.y} r={22} fill="rgba(52,211,153,0.08)" stroke="#34d399" strokeWidth={1.5} />
          <text x={assistantPos.x} y={assistantPos.y - 2} fill="#34d399" fontSize={10} textAnchor="middle">助手</text>
          <text x={assistantPos.x} y={assistantPos.y + 11} fill="#34d399" fontSize={7.5} textAnchor="middle">Harness</text>
        </g>

        {/* 用户节点 */}
        <g className="node">
          <circle cx={CX} cy={CY} r={28} fill="rgba(34,211,238,0.12)" stroke="#22d3ee" strokeWidth={2} />
          <text x={CX} y={CY - 2} fill="#e4f9ff" fontSize={11.5} textAnchor="middle" fontWeight={700}>
            {personaName ?? '用户'}
          </text>
          <text x={CX} y={CY + 12} fill="#67e8f9" fontSize={8.5} textAnchor="middle">{userLoc}</text>
        </g>
      </svg>
